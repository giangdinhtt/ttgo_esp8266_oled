...
unsigned long oled_refresh_timer = 0;
const char OLED_DUCKDNSSTATUS[] PROGMEM = "Status: {s}";
const char OLED_DUCKDNSLAST[] PROGMEM = "Last: {s}s ago";
const char OLED_DUCKDNSNEXT[] PROGMEM = "Next: in {s}s";
...
int ddnsUpdate() {
  ...
  do {
    HTTPClient http;
    http.begin(wifiClient, getrequest);
...
String oled_last_ssid;
String oled_last_ip;
String oled_last_status;
void updateOled() {
  //get last update status
  String lastupdatestatus = "Unknown";
  if(ddns_lastupdatestatus)
  ...
  //set oled content
  String status = FPSTR(OLED_DUCKDNSSTATUS);
  status.replace("{s}", lastupdatestatus);
  String lastUpdate = FPSTR(OLED_DUCKDNSLAST);
  lastUpdate.replace("{s}", String(lastupdatetime / 1000));
  String nextUpdate = FPSTR(OLED_DUCKDNSNEXT);
  nextUpdate.replace("{s}", String(nextupdatetime / 1000));

  //display oled content
  //display.clear();
Serial.println("Update OLED: ");

Serial.print("---- ");
Serial.println(WiFi.SSID());
  if (WiFi.SSID() != oled_last_ssid) {
    oled_last_ssid = WiFi.SSID();
    display.clear_row();
    display.print(&oled_last_ssid[0]);
  }

Serial.print("---- ");
Serial.println(WiFi.localIP());
  if (WiFi.localIP().toString() != oled_last_ip) {
    oled_last_ip = WiFi.localIP().toString();
    display.clear_row(1);
    display.print(&oled_last_ip[0], 1);
  }

Serial.print("---- ");
Serial.println(status);
  if (status != oled_last_status) {
    oled_last_status = status;
    display.clear_row(2);
    display.print(&oled_last_status[0], 2);
  }

Serial.print("---- ");
Serial.println(lastUpdate);
//  display.clear_row(3);
//  display.print(&lastUpdate[0], 3);

Serial.print("---- ");
Serial.println(nextUpdate);
  display.clear_row(3);
  display.print(&nextUpdate[0], 3);
...
void setup() {  
  ...
  // high level signal on D2 cause working OLED
...
void loop() {
  ...
  if (millis() > oled_refresh_timer) {
    oled_refresh_timer = millis() + 1000;
    updateOled();
  }
}
